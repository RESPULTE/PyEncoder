...
def convert_bits_to_bytes(bits: str, size: int = 0) -> bytes:
    return int.to_bytes(int(bits, 2), size or -(-len(bits) // 8), Settings.ENDIAN)


def convert_bytes_to_bits(_bytes: bytes, size: int = 0) -> str:
    return "{num:0{bit_size}b}".format(num=int.from_bytes(_bytes, Settings.ENDIAN), bit_size=size)


def convert_ints_to_bytes(_ints: int, size: int = 0) -> bytes:
    return int.to_bytes(_ints, size or -(-_ints.bit_length() // 8), Settings.ENDIAN)


def convert_bytes_to_ints(_bytes: bytes) -> int:
    return int.from_bytes(_bytes, Settings.ENDIAN)
